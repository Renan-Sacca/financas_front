...
import { ClipboardList, ChevronRight, CheckCircle, XCircle, Clock, FileText } from "lucide-react";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-heading font-bold text-white">Extratos Pendentes</h1>
          <p className="text-sm text-white/40">Revise e confirme as transações importadas antes de salvar</p>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="glass-panel rounded-2xl p-12 text-center">
          ...
          <p className="text-white/40 text-sm">Nenhum extrato pendente. Envie um arquivo pelo chat.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {batches.map((b) => {
            ...
            return (
              <button
                key={b.batch_id}
                onClick={() => navigate(`/pending/${b.batch_id}`)}
