...
import { cardsApi, banksApi } from "@/services/api";
...
export default function CardsPage() {
  ...
  const load = async () => {
    try {
      const [c, b] = await Promise.all([cardsApi.list(), banksApi.list()]);
      setCards(c);
      setBanks(b);
    } catch {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) {
        ...
        await cardsApi.create(parseInt(form.bank_id), data);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {cards.map((card) => (
            ...
            >
              ...
              <h3 className="font-heading text-xl font-medium text-white tracking-tight mb-1">
                {card.name}
              </h3>
